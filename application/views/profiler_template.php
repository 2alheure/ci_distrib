...
<style type="text/css">
	...
	.ci-profiler-box h2 { font-family: Helvetica, sans-serif; font-weight: bold; font-size: 16px !important; padding: 0; line-height: 2.0; background-color: inherit; }
	...
	#codeigniter-profiler table.main td pre { color: white !important; }
	...
	#codeigniter-profiler table .hilight { color: #FFFD70 !important; cursor: pointer; }
		...
		<div id="ci-profiler-console" class="ci-profiler-box" style="display: none">
			...
			<?php if (is_array($sections['console'])) : ?>
				...
				<?php foreach ($sections['console']['console'] as $log) : ?>
					...
					<?php else : ?>
						<tr>
							<td class="hilight" onclick="ci_profiler_bar.toggle('toggle-console-<?= $log['type'] ?>')"><?php echo $log['type'] ?></td>
							<td id="toggle-console-<?= $log['type'] ?>"><pre><?php echo htmlspecialchars($log['data']) ?></pre></td>
							<td></td>
						</tr>
					<?php endif; ?>
				<?php endforeach; ?>
			...
			<?php if (isset($sections['userdata'])) :?>
					...
					<table class="main" id="userdata_table">
						<?php if (is_array($sections['userdata']) && count($sections['userdata'])) : ?>
							...
							<?php endforeach; ?>
						<?php else : ?>
							<tr><td>Aucune donnée SESSION n'existe</td></tr>
						<?php endif; ?>
					</table>
				<?php endif; ?>
			...
			<?php if (isset($sections['view_data'])) : ?>
				...
				<?php if (is_array($sections['view_data'])) : ?>
					...
					<?php foreach ($sections['view_data'] as $key => $val) : ?>
						<tr><td class="hilight" onclick="ci_profiler_bar.toggle('toggle-view_data-<?= $key ?>')"><?php echo $key ?></td><td id="toggle-view_data-<?= $key ?>"><?php echo $val ?></td></tr>
					<?php endforeach; ?>
					</table>
